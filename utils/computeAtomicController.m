...
try
    rho_if = varargin{1};
    [ac,c] = computeAtomicControllerSegment(u0,x0,sys,regMode,ellToCompose,rho_if,options);
    ...
    if strcmp(ME.identifier, 'Drake:PolynomialTrajectorySystem:InfeasibleRho')
        if length(x0) > 10
            ...
            [ac2, c2] = computeAtomicController(u02,x02,sys,regMode,ellToCompose,options,rho_if);
            t0 = ac2.P.getTimeVec();
            ...
            [ac1, c1] = computeAtomicController(u01,x01,sys,regMode,ellToCompose,options,rho_if);
            t0 = ac1.P.getTimeVec();
