...
This package requires MATLAB 2012b or later.  In addition, it requires installation of the following dependencies.  It may be convenient to install these within the 'lib' folder in the project directory, to make the path set-up easier.

1) Either of the following:
   a) Mosek (preferred):
      https://www.mosek.com/resources/downloads
      A license can requested free of charge for academic users or purchased for non-academic users.

   b) SeDuMi (version 1.3 tested):
...
3) Multi-Parametric Toolbox (version 2.6.2 tested) 
   http://people.ee.ethz.ch/~mpt/2/downloads/

3) Drake (either Binary or Source installation):
...
With the workspace variables loaded, execute the following scripts:
